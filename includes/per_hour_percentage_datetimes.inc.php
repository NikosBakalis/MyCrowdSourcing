...
if ($_SESSION['type'] == 'admin'){
  ...
  $sql_3 = "SELECT timestamp_a FROM activity_details WHERE timestamp_a > '$start_date' AND timestamp_a < '$end_date'"; // This one right here selects everything from activity_details table. I want only the count but I will retrieve it with mysqli_num_rows() function.
} else {
  ...
  $sql_3 = "SELECT timestamp_a FROM activity_details WHERE userID = '$current_userID' AND timestamp_a > '$start_date' AND timestamp_a < '$end_date'"; // This one right here selects everything from activity_details table. I want only the count but I will retrieve it with mysqli_num_rows() function.
}
...
if (!mysqli_stmt_prepare($stmt, $sql_1) || !mysqli_stmt_prepare($stmt, $sql_3)) { //This one right here will check if the sql statement above working properly.
  ...
  while($resultCheck_3 = mysqli_fetch_array($result_3)){
    ...
    if(!in_array($hourLetter, $hourArray, true)){
      if ($_SESSION['type'] == 'admin'){
        $sql_2 = "SELECT * FROM activity_details WHERE timestamp_a LIKE '____-__-__ $hourNumber:__:__' AND timestamp_a > '$start_date' AND timestamp_a < '$end_date'"; // This one right here selects everything from activity_details table where type is something that the admin choose in frontend. I want only the count but I will retrieve it with mysqli_num_rows() function.
      } else {
        $sql_2 = "SELECT * FROM activity_details WHERE timestamp_a LIKE '____-__-__ $hourNumber:__:__' AND userID = '$current_userID' AND timestamp_a > '$start_date' AND timestamp_a < '$end_date'"; // This one right here selects everything from activity_details table where type is something that the admin choose in frontend. I want only the count but I will retrieve it with mysqli_num_rows() function.
      }
