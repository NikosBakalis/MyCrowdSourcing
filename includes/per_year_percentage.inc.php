...
if (!mysqli_stmt_prepare($stmt, $sql_1) || !mysqli_stmt_prepare($stmt, $sql_3)) { //This one right here will check if the sql statement above working properly.
  ...
  while($resultCheck_3 = mysqli_fetch_array($result_3)){
    ...
    $yearNumber = date("Y", strtotime($timestamp_a));
    $yearLetter = date("Y", strtotime($timestamp_a));
    if(!in_array($yearLetter, $yearArray, true)){
      $sql_2 = "SELECT * FROM activity_details WHERE timestamp_a LIKE '$yearNumber-__-__ __:__:__'"; // This one right here selects everything from activity_details table where type is something that the admin choose in frontend. I want only the count but I will retrieve it with mysqli_num_rows() function.
      ...
      array_push($yearArray, $yearLetter); // This one right here pushes to the array the type of the activity
